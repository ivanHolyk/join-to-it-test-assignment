...
function handleSubmit() {
  ...
  const payloadStart = startDatetime.value
  const payloadEnd = endDatetime.value
...
async function onToggleAllDay(checked: boolean) {
  ...
  await nextTick()
  if (eventStore.selectedPosition) {
...
watch(
  () => [(eventStore as any).selectedDate, (eventStore as any).selectedPosition],
  async () => {
    await nextTick()
    computeAndApplyPosition()
  },
...
watch(
  () => (eventStore as any).selectedDate,
  (newSelected) => {
    if (!newSelected) return

    if (isDateOnly(String(newSelected))) {
      allDay.value = true
      startDatetime.value = String(newSelected)
      endDatetime.value = String(newSelected)
    } else {
      allDay.value = false
      startDatetime.value = String(newSelected)
      endManuallyEdited.value = false
...
<template>
  <div class="modal-backdrop" @click.self="handleCancel" role="dialog" aria-modal="true">
    <div
      class="modal-card"
      ref="cardRef"
      :style="cardStyle"
